...
# 
# FIXME Patched Makefile's have a hard-coded "target" folder
# Work-around with symlink to pkgdir
#
...
md5sums=(
  7f4487f62245292a383e99865ee5b71b
...
package_codaj12() {
  provides=(libcodadec.so)
  pkgdesc="JPEG hardware Codec IP from Chip&Media performs JPEG baseline/extended sequential and M-JPEG decoding and encoding"
...
package_wave420l() {
  provides=(libsfenc.so)
  pkgdesc="HEVC/H.265 hardware encoder IP from Chip&Media encoding FHD/UHD HEVC/H.265 main profile L4.1"
  license=('LGPL-2.1' 'BSD-3-Clause')
  ln -s "${pkgdir}" "${srcdir}/${pkgbase}_v${pkgver}/target"
...
package_wave511() {
  provides=(libsfdec.so)
  pkgdesc="4K multi-format hardware decoder IP from Chip&Media with support for both HEVC/H.265 and AVC/H.264"
  license=('LGPL-2.1' 'BSD-3-Clause')
  ln -s "${pkgdir}" "${srcdir}/${pkgbase}_v${pkgver}/target"
...
package_omx-il() {
  provides=(libsf-omx-il.so libOMX_Core.so)
  pkgdesc="OpenMAX IL (Integration Layer) for hardware video encoding/decoding, JPEG/M-JPEG decoding through the codaj12/wave511/wave420l library"
  license=('GPL-2.0')
